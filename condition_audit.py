...
def construct_query(table, entry, timestamp, sites):
    try:
        ...
        query = f'SELECT "dbo"."{table}"."{entry["Column"]}" FROM "dbo"."{table}"'
         # if entry["Table"] == "ER_L_QciProfilePredefined":
        ...
        if entry["Table"] == "ER_L_QciProfilePredefined" and entry["Column"] == "drxProfileRef":
            query = f'SELECT SUBSTRING("dbo"."{table}"."{entry["Column"]}" FROM \'DrxProfile=[0-9]+\') AS "drxProfileRef" FROM "{table}"'
        ...
        #     query = f'SELECT SUBSTRING("dbo"."{table}"."{entry["Column"]}" FROM \'vsDataQciTable=default,vsDataLogicalChannelGroup=[0-9]+\' FOR \'vsDataQciTable=default,vsDataLogicalChannelGroup=1\') AS "logicalChannelGroupRef" FROM "dbo"."{table}"'
        else:
            query = f'SELECT "dbo"."{table}"."{entry["Column"]}" FROM "dbo"."{table}"'
        ...
        if conditions:
            for i, (condition_table, condition_column, condition_value, condition_type) in enumerate(conditions):
                if condition_type == "RANGE":
                    query += f'''
                    JOIN "dbo"."{condition_table}" AS ct{i} ON "dbo"."{table}"."{entry["Join_Column"]}" = ct{i}."DimensionId" AND {condition_column} {condition_value}
                    '''
        ...
        if entry["Table"] == "ER_L_QciProfilePredefined" and entry["Column"] == "drxProfileRef":
            if "WHERE" in query:
                query += f' AND "dbo"."{table}"."qciProfilePredefinedId" = \'{entry["Condition_Table_1_Column_Value"]}\''
            else:
