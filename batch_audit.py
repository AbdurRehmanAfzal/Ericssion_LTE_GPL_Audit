...
data = [
    ...
    {
        "Table": "ER_L_ENodeBFunction",
        "Column": "schedulingStrategy",
        "Column_Golden_Value": "ROUND_ROBIN"
    },
    {
        "Table": "ER_L_ENodeBFunction",
...
def write_to_csv(results):
    try:
        with open('batch_debug_audit_results.csv', mode='w', newline='') as file:
...
def write_to_log(errors):
    try:
        with open('batch_query_errors.log', mode='w') as file:
            for error in errors:
                file.write(f"Query: {error['query']}\nError: {error['error_message']}\n")
    except IOError as e:
        print(f"Error writing to log file: {e}")

def construct_query(entry):
    try:
        table = entry["Table"]
        column = entry["Column"]
        conditions = entry.get("Conditions", [])
        
        where_clauses = [f'"{entry["Join_Column"]}" = ct{i}."DimensionId" AND ct{i}."{cond["Condition_Column"]}" = \'{cond["Condition_Value"]}\''
                         for i, cond in enumerate(conditions)]
        
        joins = [f'JOIN "{cond["Condition_Table"]}" AS ct{i} ON "{table}"."{entry["Join_Column"]}" = ct{i}."DimensionId"'
                 for i, cond in enumerate(conditions)]
        
        where_clause = ' AND '.join(where_clauses)
        join_clause = ' '.join(joins)
        
        query = f'SELECT "{table}"."Level", "{table}"."{column}" FROM "{table}" {join_clause} WHERE {where_clause}'
...
def main():
    ...
    queries = []
    for entry in data:
        if entry["Table"] in tables or tables == ["all"]:
            query = construct_query(entry)
            if query:
    ...
    for query in queries:
        ...
        for result in results:
            ...
            entry = next(e for e in data if e["Table"] in query and e["Column"] in query)
            if entry["Column_Golden_Value"] != current_value:
                audit_results.append([level, entry["Table"], entry["Column"], entry["Column_Golden_Value"], current_value])
